(function () {
    ...
    function cacheElements() {
        [
            ...
            'fix-orientation', 'preset', 'results-container', 'results-list', 'results-table-wrap',
            'results-table-body', 'download-all-btn', 'clear-all-btn', 'batch-summary',
            'batch-count', 'batch-saved', 'batch-avg', 'batch-progress-bar', 'empty-results',
    ...
    function renderTaskUI(task) {
        ...
        card.innerHTML = `
            ...
            <p class="error-msg text-red-400 text-sm hidden" role="alert"></p>
            <div class="compare-container bg-checker" role="slider" aria-label="Compare original and compressed" aria-valuemin="0" aria-valuemax="100" aria-valuenow="50" tabindex="0">
                <img src="${task.originalUrl}" class="compare-img original-img opacity-50" alt="Original">
                <div class="loading-overlay"><div class="spinner"></div></div>
                <div class="compare-overlay hidden"><img class="compare-img compressed-img" alt="Compressed"></div>
                <div class="compare-handle hidden" aria-hidden="true"></div>
            </div>
    ...
    function renderTableRow(task) {
        ...
        tr.id = `row-${task.id}`;
        tr.innerHTML = `
            <td class="truncate max-w-[180px]" title="${escapeHtml(task.file.name)}">${escapeHtml(task.file.name)}</td>
            ...
            <td class="table-actions">
                ...
                <a class="btn-link download-row hidden" download>Save</a>
            </td>
        `;
    ...
    function updateTaskUI(task) {
        ...
        card.querySelector('.original-img')?.classList.remove('opacity-50');
        ...
        card.querySelector('.result-actions').classList.remove('hidden');
    ...
    function recompressTask(id) {
        ...
        if (card) {
            ...
            card.querySelector('.result-actions').classList.add('hidden');
            ...
            card.querySelector('.compare-overlay')?.classList.add('hidden');
            card.querySelector('.compare-handle')?.classList.add('hidden');
        }
        updateTaskStatus(id, 'Processing…', 'processing');
        drainQueue();
    ...
    function showResultsArea() {
        ...
        els['empty-results'].classList.add('hidden');
        els['batch-summary'].classList.remove('hidden');
        els['clear-all-btn'].classList.remove('hidden');
    }

    function hideResultsIfEmpty() {
        els['results-container'].classList.add('hidden');
        els['empty-results'].classList.remove('hidden');
        els['batch-summary'].classList.add('hidden');
        els['clear-all-btn'].classList.add('hidden');
        els['download-all-btn'].classList.add('hidden');
    }
    ...
    function updateBatchDownloadBtn() {
        ...
        els['download-all-btn'].classList.toggle('hidden', n < 2);
    }
    ...
    function setViewMode(mode) {
        ...
        els['results-list'].classList.toggle('hidden', !cards);
        els['results-table-wrap'].classList.toggle('hidden', cards);
        saveSettings();
    }
    ...
    function applyTheme(theme) {
        ...
        els['theme-toggle'].textContent = theme === 'dark' ? 'Light theme' : 'Dark theme';
